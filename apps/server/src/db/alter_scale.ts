...
async function run() {
  ...
  try {
    ...
    await db.execute(
      sql`ALTER TABLE "inventory_items" ALTER COLUMN "current_stock" TYPE numeric(14, 6);`,
    );
    await db.execute(
      sql`ALTER TABLE "inventory_items" ALTER COLUMN "reorder_level" TYPE numeric(14, 6);`,
    );
    await db.execute(
      sql`ALTER TABLE "inventory_items" ALTER COLUMN "cost_per_unit" TYPE numeric(14, 6);`,
    );
    await db.execute(
      sql`ALTER TABLE "inventory_adjustments" ALTER COLUMN "quantity_delta" TYPE numeric(14, 6);`,
    );
